...
namespace BulkBatcher.Models
{
    ...
    public sealed class NullableInt32JsonConverter : JsonConverter<int?>
    {
        private const NumberStyles IntegerStyles =
            NumberStyles.Integer | NumberStyles.AllowThousands | NumberStyles.AllowCurrencySymbol;

        // InvariantCulture's currency symbol is "\u00A4", so clone its number format
        // and use "$" instead to accept values like "$1,200" from spreadsheets.
            ...
            switch (reader.TokenType)
            {
                ...
                case JsonTokenType.String:
                    ...
                    if (string.IsNullOrWhiteSpace(s))
                        return null;
                    if (int.TryParse(s, IntegerStyles, NumberFormat, out var parsed))
                        return parsed;
